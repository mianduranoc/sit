...
// empieza validar lugar
        ...
        case 'modificar':
            ...
            $alumnos=mysqli_query($db->getConnection(),"SELECT * FROM Tutorado_Conferencia WHERE id_conferencia='$id'");
            $conferencia=mysqli_query($db->getConnection(),"SELECT * FROM Conferencia WHERE id_conferencia=$id");
            $conferencia=mysqli_fetch_assoc($conferencia);
            $hilo=new Hilo($alumnos,$conferencia);
            $hilo->run();
            //die();
            // echo mysqli_error($conexion);
            // echo $fecha;
