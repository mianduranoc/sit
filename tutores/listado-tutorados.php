					...
					<div class=" container-fluid" id="listado-tutorados">
						<div class="col-md-12 row">
							<div class="row titulo-listado-tutorados">Listado de Tutorados</div>
								...
								<table class="table table-hover">
					                ...
					                <tbody class="renglones" >
						                ...
						                <tr class = "prueba">
											...
											<?php  
												$aux = mysqli_query($conexion,"SELECT desertor FROM Alumno_Tutoria WHERE nc = '".$alumno['nc']."'");
												if(mysqli_num_rows($aux)):
													$des = mysqli_fetch_assoc($aux);
													if($des['desertor'] == 1):
											?>
													<td><input type="checkbox" name="desertor[]" value = <?=$alumno['nc']?> checked ="checked" class = "desertores"></td>
												<?php 
													 else:
												?>
													<td><input type="checkbox" name="desertor[]" value = <?=$alumno['nc']?>></td>
						                		<?php endif;else:?>
						                				<td><input type="checkbox" name="desertor[]" value = <?=$alumno['nc']?>></td>
						                </tr>
					                	<?php 		
					            ...
					            <div id="id"><input type="submit" name = "desercion" value = "Guardar" class = "btn btn-primary" id ="guardar"></div>
					        </form>
					    </div>
					</div> 
				</div>
